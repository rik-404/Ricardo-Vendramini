        ...
        >
          ...
          <div className="p-6 sm:p-8 border-b border-white/10 shrink-0 bg-zinc-900/90 relative">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-mono text-zinc-400 uppercase tracking-widest">{t('certificates.allGallery')}</span>
            </div>
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-xs font-mono transition-colors ${
                      activeFilter === cat
                        ...
                        : 'bg-black/60 text-zinc-300 hover:text-white border border-white/10 hover:border-white/40'
                    }`}
                  >
                    {categoryLabel(cat)}
                  </button>
          ...
          <div className="p-6 sm:p-8 overflow-y-auto flex-1">
            {filteredCertificates.length === 0 ? (
              <div className="text-center py-16">
                <p className="text-zinc-400 font-mono text-sm">{t('certificates.noResults')}</p>
                <button
                  onClick={() => {
                    setActiveFilter('all');
                    setSearchQuery('');
                  }}
                  className="mt-4 px-4 py-2 rounded-xl bg-white/10 border border-white/20 text-zinc-200 text-xs font-mono hover:bg-white hover:text-zinc-950 transition-colors"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCertificates.map((cert) => (
                  <div
                    ...
                    className="glass-card rounded-2xl overflow-hidden border border-white/10 hover:border-white/30 group cursor-pointer flex flex-col justify-between bg-zinc-900/40 transition-all"
                  >
                    <div>
                      {/* Image Container */}
                      <div className="relative h-48 overflow-hidden bg-zinc-950">
                        ...
                        {/* Top Badge */}
                        <div className="cert-official-badge absolute top-3 left-3 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full glass-panel border border-white/20 text-zinc-300 text-[11px] font-mono font-semibold">
                      ...
                      <div className="p-5">
                        ...
                        <h3 className="text-lg font-bold text-white mb-1 group-hover:text-zinc-200 transition-colors truncate">
                          {cert.title}
                        </h3>
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {cert.skills.slice(0, 3).map((sk, idx) => (
                            <span key={idx} className="px-2 py-0.5 rounded bg-zinc-900 border border-white/10 text-zinc-300 text-[10px] font-mono">
                              {sk}
                            </span>
                          ))}
                    ...
                    <div className="p-5 pt-0">
                      <button
                        ...
                        className="w-full py-2 px-3 rounded-xl bg-white/10 hover:bg-white text-zinc-200 hover:text-zinc-950 font-mono text-xs font-semibold tracking-wider uppercase transition-colors border border-white/15 flex items-center justify-center gap-2"
                      >
